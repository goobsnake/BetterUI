...
function BETTERUI.Inventory.Settings.RegisterPanel(mId, moduleName)
	...
	local function ResetInventoryGeneralSettings()
		if not (BETTERUI.CIM.Settings and BETTERUI.CIM.Settings.ResetModuleSettingsByGroup
				and BETTERUI.CIM.Settings.ResetModuleSettingsByGroup("Inventory", "general")) then
	...
	if BETTERUI.CIM.Settings and BETTERUI.CIM.Settings.RegisterModulePanel then
		local panelId, panelReason = BETTERUI.CIM.Settings.RegisterModulePanel(mId, panelData, optionsTable)
		if panelId then
			return true, panelId
		end
