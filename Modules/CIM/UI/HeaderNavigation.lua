...
Mechanism:
  ...
  3. Sets flag to prevent duplicate saves.
...
function BETTERUI.CIM.HeaderNavigation.CycleCategory(instance, delta, options)
    ...
    instance._cyclingCategory = true
    ...
    instance._cyclingCategory = false
end
...
Mechanism:
  ...
  2. Uses token-based coalescing to ignore stale timer callbacks.
...
function BETTERUI.CIM.HeaderNavigation.CreateCoalescedHandler(options)
    ...
    return function(instance, list, selectedData)
        -- Skip during mode toggle
        if instance._justToggledMode then return end
        -- Skip during header callback suppression
        if instance._suppressHeaderCallback then return end

        -- Save position BEFORE switching (unless CycleCategory already did)
        if not instance._cyclingCategory and options.onSave then
        ...
        -- Token-based coalescing
        instance._categoryChangeToken = (instance._categoryChangeToken or 0) + 1
        local myToken = instance._categoryChangeToken

        -- Suppress updates during coalesce window
        instance._suppressListUpdatesToken = myToken
        instance._suppressListUpdates = true

        zo_callLater(function()
            -- Check if scene is still visible
            if options.sceneCheck and not options.sceneCheck() then
                if instance._suppressListUpdatesToken == myToken then
                    instance._suppressListUpdates = false
                    instance._suppressListUpdatesToken = nil
                end
                return
            end

            -- Check if this is still the latest change
            if myToken ~= instance._categoryChangeToken then
                return
            end

            -- Clear suppression and apply change
            if instance._suppressListUpdates and instance._suppressListUpdatesToken == myToken then
                instance._suppressListUpdates = false
                instance._suppressListUpdatesToken = nil
            end
