...
local function TraceNeedsReviewKeybind(phase, data)
    ...
    data = data or {}
    data.fn = data.fn or "BuilogCommands.NeedsReviewKeybind"
    data.scene = data.scene or GetActiveSceneName()
    local category = (L.CATEGORY and (L.CATEGORY.KEYBIND or L.CATEGORY.LIFECYCLE)) or "KEYBIND"
...
local function DispatchNeedsReviewKeybind(origin)
    ...
    TraceNeedsReviewKeybind(ok and "end" or "failed", {
        origin = origin,
        requestId = ok and requestId or nil,
        error = ok and nil or tostring(requestId),
    })
    if ok then
        return requestId, layout, screenshot
    end
...
BuilogCommands.FlagNeedsReview = function()
    return DispatchNeedsReviewKeybind("global-binding")
end
