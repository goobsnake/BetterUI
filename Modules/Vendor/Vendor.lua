...
local VendorKeybinds = Vendor.Keybinds
assert(type(VendorKeybinds) == "table", "VendorKeybinds.lua must load before Vendor.lua")
...
local function DescribeVendorKeybinds(instance)
    return VendorKeybinds.Describe(instance)
end
...
function TraceVendorEvent(event, phase, data, category)
    ...
    local inputAnchorDetail = data._inputAnchorDetail == true
    data._inputAnchorDetail = nil
    if inputAnchorDetail then
        data.module = nil
    else
...
local function BuildCoreKeybinds(vendorInstance)
    return VendorKeybinds.BuildCoreKeybinds(vendorInstance, {
        Vendor = Vendor,
        MODE = MODE,
        traceVendorEvent = TraceVendorEvent,
        getActiveTabs = GetActiveTabs,
        getToggleModePair = GetToggleModePair,
        getCurrentVendorTargetData = GetCurrentVendorTargetData,
        isPrimaryActionAllowed = IsPrimaryActionAllowed,
        canMultiSelectInCurrentMode = function() return CanMultiSelectInCurrentMode() end,
        isMultiSelectAvailable = function() return IsMultiSelectAvailable() end,
        registerVendorBatchDialog = function() return RegisterVendorBatchDialog() end,
        isFenceInteraction = function() return isFenceInteraction end,
        isStableInteraction = function() return isStableInteraction end,
    })
end
