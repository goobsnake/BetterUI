...
param(
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Skip generating mipmaps (Mipmaps are generated by default)")]
    [switch]$SkipMipmaps,
...
function Get-FormatArgs {
    ...
    # Use standard UNORM formats to preserve original color values (avoid gamma shift)
    switch ($Format) {
        "DXT1" { return @("-f", "BC1_UNORM") }
        "DXT3" { return @("-f", "BC2_UNORM") }
        "DXT5" { return @("-f", "BC3_UNORM") }
        "BGRA" { return @("-f", "B8G8R8A8_UNORM") }
        default { return @("-f", "BC3_UNORM") }  # Default to DXT5
    }
}
...
function Convert-TextureToDds {
    ...
    $args += "-if", "CUBIC" # High-quality filtering (Cubic) for resizing/mipmaps
    $args += "--ignore-srgb" # Ignore input gamma to preserve color values "as is" when converting PNG->UNORM
    
    if ($SkipMipmaps) {
        $args += "-m", "1"  # No mipmaps (single level)
    }
    else {
        # Default behavior: Generate mipmaps automatically
        # texconv generates full chain by default when -m is not specified
        if ($Format -ne "BGRA") {
            # For compressed formats, ensure we have enough levels
        }
    }
...
Write-Host "  Texture to DDS Converter for ESO (PNG/DDS)" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Yellow
Write-Host ""
...
$files = @()
if (Test-Path $InputPath -PathType Container) {
    $files = Get-ChildItem -Path $InputPath -Include "*.png", "*.dds" -Recurse -File
...
foreach ($file in $files) {
    $result = Convert-TextureToDds -InputFile $file.FullName -OutputDirectory $OutputDir -TexconvExe $texconv -Format $Format -SkipMipmaps $SkipMipmaps -Pow2Resize $ResizePow2
    if ($result) {
        $successCount++
    }
    else {
