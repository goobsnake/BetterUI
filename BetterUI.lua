...
-- Debug command to diagnose DIRECTIONAL_INPUT issues (developer use only)
-- Set BETTERUI_DEBUG = true to enable
if BETTERUI_DEBUG then
	SLASH_COMMANDS["/buidebug"] = function()
		if not DIRECTIONAL_INPUT then
			d("[BetterUI] DIRECTIONAL_INPUT not available")
			return
		end
		...
		d("[BetterUI] DIRECTIONAL_INPUT Debug - " .. #inputObjects .. " objects registered:")
		for i, obj in ipairs(inputObjects) do
			...
			local isBetterUI = false
			if controlName and controlName:find("BETTERUI") then
				isBetterUI = true
			end

			d(string.format("  [%d] %s - Control: %s %s",
				i,
				objType,
				controlName,
				isBetterUI and "(BETTERUI)" or ""))
		end

		-- Also check if any input devices are consumed
		d("[BetterUI] Input device consumed state:")
		for device = 1, 5 do
			...
			d(string.format("  %s: %s", deviceName, consumed and "CONSUMED" or "available"))
		end
	end
end
