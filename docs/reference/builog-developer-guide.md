...
- Contract tests: `tools/tests/test_log_parse_contract.lua`, `test_builog_flow_source.lua`,
  `test_builog_monitor_source.lua`, `test_vendor_trace_source.lua`,
...
`requested`, `begin`, `end`, `completed`, `confirmed`, `settled`, `blocked`, `failed`,
`skipped`, `pending`, `changed`, `snapshot`, `queued`, `executed`, `expired`, `step`,
and `abort`.
