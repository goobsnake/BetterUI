...
| `TRC-006` | 2026-06-12 | Guild store browse | Browse and purchase guild-specific store items (e.g. guild tabards) | Low | Medium | P3 | Open | Engineer review (MPR-3, reviewed & deferred). BetterUI's TradingHouse has no `GetGuildSpecificItemInfo` / `BuyGuildSpecificItem` path, so guild-specific items are not browsable or purchasable. Niche scope gap; likely intentional. |
...
| ID | Date | Area | Request | Impact | Effort | Priority | Status | Notes |
|---|---|---|---|---|---|---|---|---|
| `PLT-001` | 2026-06-11 | Code structure | Split files exceeding the 600-line standard by concern | Medium | High | P3 | Open | Engineer review (MPR-1). `Vendor/Core/VendorClass.lua` (~1840: header model, input ownership, search lifecycle), `Vendor/Vendor.lua` (~1390: keybinds, dialogs), `Banking/Core/BankingClass.lua` (~1067: sort comparators, transfer DI), `Inventory/Keybinds/CraftBagKeybinds.lua` (807; misnamed — holds generic inventory keybind logic), `GeneralInterface/Tooltips/Tooltips.lua` (~750), `CIM/Core/Batching/MultiSelectMixin.lua` (735; ~500-line `ProcessBatchThrottled`), `ResourceOrbFrames/Core/OrbVisuals.lua` (731), `CIM/Core/Diagnostics/DebugCommands.lua` (704), `CIM/Core/Batching/BatchConfig.lua` (613), plus the ~490-line `UpdateTooltipEquippedText` in `Inventory/UI/TooltipEquipped.lua`. **Reviewed 2026-06-12 (MPR-2): deferred** — splitting these large files (esp. `VendorClass.lua` ~1838) is a high-regression-risk refactor with no functional benefit that needs in-game (L4) validation; tracked for a dedicated, separately-validated pass rather than bundled with behavioral fixes. |
| `PLT-002` | 2026-06-11 | Localization | Backfill non-EN locale files with missing string ids | Medium | Medium | P3 | Open | Engineer review (MPR-1). MPR-2 (2026-06-12) measured the exact gap: **83 `SI_BETTERUI_*` ids** exist only in `lang/en.lua` and are absent from **all six** non-EN locales (de/es/fr/jp/ru/zh), identical set per locale, zero orphans, no es delta. **Confirmed cosmetic**: `BetterUI.txt` loads `lang/en.lua` then `lang/$(language).lua`, so missing ids render in English at runtime (no functional bug). **Deferred to native/human translation**: machine-translating ~498 ESO-specific UI strings (format specifiers, gender/plural, ESO terminology) risks shipping low-quality strings unfit for release; the full id inventory is captured for a translator. Backfill via the preview-first `l10n_*` flow when translations are available. |
...
| `PLT-005` | 2026-06-12 | Code structure | Optionally harden Banking/Inventory transfer batches to re-validate with `IsSlotIdentityCurrent` instead of `HasItemAtSlot` | Low | Low | P3 | Open | Engineer review (MPR-3, reviewed & deferred). Banking/Inventory transfer batches re-check with `HasItemAtSlot` rather than the stricter `IsSlotIdentityCurrent`. This is a consistent cross-cutting pattern for non-destructive moves (the move is itself harmless if the slot shifted), so it is acceptable as-is; tracked only as optional future hardening. |
| `PLT-006` | 2026-06-19 | Accessibility | Harden + broaden gamepad narration: wrap the `canNarrate`/`selectedNarrationFunction` callback bodies (not just the registration call) so screen-reader invocations can't throw, and extend coverage beyond title + selected item to footer currency, mode/category changes, and action/keybind labels on focus changes | Medium | Medium | P3 | Open | Improvement-cycle security/a11y review (codex). `NarrationHelper.lua:159-191` protects only the registration call and emits only title + selected item; currency/mode/category helpers exist but are not integrated. |
| `PLT-007` | 2026-06-19 | Diagnostics / logging | Optional batch-coalescing mode for `verbose` logging: pack multiple breadcrumbs into one engine error to cut per-line traceback amplification and disk churn on the Interface.log deferred-error sink | Low | Medium | P3 | Open | Improvement-cycle logging review (codex). The per-frame/second budget added in v3.07 bounds frame cost; coalescing would further reduce IO during high-volume verbose sessions, at the cost of external tail parsers needing to understand batch records. |
...
| `HUD-004` | 2026-06-12 | Resource orbs / cooldowns | Filter the global cooldown by reading the `global` (3rd) return of `GetSlotCooldownInfo` instead of a 1500ms threshold | Low | Low | P3 | Open | Engineer review (MPR-3, reviewed & deferred). `CooldownUtils` uses a 1500ms threshold to filter the GCD rather than the `isGlobalCooldown` return of `GetSlotCooldownInfo`, inconsistent with `FrontBarManager` which already reads `isGlobalCooldown`. Deferred — changing visible cooldown behavior needs in-game (L4) validation; the affected edge case is minor (real 1000–1500ms non-GCD cooldowns). |
