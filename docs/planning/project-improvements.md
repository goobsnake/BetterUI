...
- [ ] Task: Banking legacy spinner-confirmation flow: remove the unreachable `confirmationMode` paths (`TransferActions.lua:949-957` always-returns guard, `BankingSceneLifecycle.lua:291-292` dead branch, never-assigned `spinnerKeybindStripDescriptor` refs at `BankingSceneLifecycle.lua:315` + commented `StateManager.lua:197`) together with `WindowClass.UpdateSpinnerConfirmation` if its callers all pass `false`. (est: 20 min)
