# Documentation Index

## Start Here

1. [contributing-guide.md](./guides/contributing-guide.md) - coding standards and contribution workflow
2. [testing-guide.md](./guides/testing-guide.md) - standalone and in-game validation checklist
3. [iterative-improvement-guide.md](./guides/iterative-improvement-guide.md) - repeatable improvement loop from intake to closeout
4. [feature-requests.md](./planning/feature-requests.md) - structured product backlog intake
5. [project-improvements.md](./planning/project-improvements.md) - active implementation plan tracker

## Architecture and Reference

- [architecture.md](./reference/architecture.md) - addon structure, module boundaries, and runtime flow
- [events-reference.md](./reference/events-reference.md) - BetterUI custom callback events and payloads
- [tribal-knowledge.md](./reference/tribal-knowledge.md) - ESO engine quirks and hard-won implementation patterns

## Specialized Guides

- [resource-orb-texture-guide.md](./guides/resource-orb-texture-guide.md) - texture pipeline and file contract for ResourceOrbFrames
- [agent-collaboration-guide.md](./guides/agent-collaboration-guide.md) - request patterns and review expectations for coding-agent execution

## Planning

- [feature-requests.md](./planning/feature-requests.md) - durable feature requests and parity gaps
- [priority-backlog.md](./planning/priority-backlog.md) - active critical/high-priority execution backlog
- [project-improvements.md](./planning/project-improvements.md) - phased implementation tracker

## Publishing Assets

- [addon-description.txt](./publishing/addon-description.txt) - ESOUI listing description (BBCode)
- [changelog.txt](./publishing/changelog.txt) - release notes for distribution

## Audience Map

- Contributors: `guides/contributing-guide.md`, `guides/testing-guide.md`, `guides/agent-collaboration-guide.md`
- Maintainers: `guides/iterative-improvement-guide.md`, `planning/priority-backlog.md`, `planning/project-improvements.md`
- Release management: `publishing/addon-description.txt`, `publishing/changelog.txt`
